...
void setup() {
  ...
  // Place sphere
  sLocation = new PVector(xCloth + 150, yCloth + 100, zCloth + 140);
  ...
  void update(float dt) {
    ...
    for (int i = 1; i < numSprings; i++) {
      for (int j = 0; j < numSprings; j++) {
        ...
        if (dist(s.location.x, s.location.y, s.location.z, sLocation.x, sLocation.y, sLocation.z) < sRadius+2) {
          ...
          s.location.set(PVector.add(sLocation, PVector.mult(norm, sRadius+2)));
        }
